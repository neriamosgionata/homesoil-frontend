<script lang="ts">
    ...
    const saveSchedule = (schedule: any) => {
        let s = {...$scripts[id]};
        s.schedule = schedule.detail || null;
        $ws.modifyScript(s);
    };
</script>
...
<div class="container">
    <div class="bg-gray-100 rounded-lg shadow-xl py-2">
        ...
        <h1 class="text-2xl text-center font-bold mb-4 text-gray-600">Sensor: {$scripts[id].title}</h1>
        ...
        <div class="m-4">
            ...
            <CronInput
                    cron={$scripts[id].schedule}
                    on:change={(e) => saveSchedule(e)}
            />
        </div>
