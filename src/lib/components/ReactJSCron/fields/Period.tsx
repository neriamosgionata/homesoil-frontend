...
export default function Period(props: PeriodProps) {
	const { value, setValue, locale, className, disabled, readOnly, shortcuts, allowedPeriods, allowClear } = props;
	...
	if (allowedPeriods.includes("minutes-steps")) {
		options.push({
			value: "minutes-steps",
			label: locale.minuteStepsOption || DEFAULT_LOCALE_EN.minuteStepsOption
		});
	}
