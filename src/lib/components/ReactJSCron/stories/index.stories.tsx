...
import { Input as AntdInput, Button, ConfigProvider, Divider, Form, Radio, Select, Switch, Table, theme } from "antd";
import type { ThemeConfig } from "antd/es/config-provider/context";
import React, { useMemo, useState } from "react";

import Cron, { type AllowEmpty, type ClockFormat, type CronError, type PeriodType } from "../index";
import "../styles.css";
import type { ClearButtonAction, CronType, Mode } from "../types";
import { ENGLISH_VARIANT_LOCALE, FRENCH_LOCALE, NO_PREFIX_SUFFIX_LOCALE } from "./constants.stories";
...
export function Demo() {
	...
	return (
		<div>
			...
			<p style={{ marginTop: 20 }}>Error: {error ? error.description : "undefined"}</p>
		</div>
	);
}
...
export function DynamicSettings() {
	...
	const [locale, setLocale] = useState<"english" | "french" | "english-variant">("english");
	...
	const [periodicityOnDoubleClick, setPeriodicityOnDoubleClick] = useState<boolean>(true);
	const [mode, setMode] = useState<Mode>("multiple");
	const [key, setKey] = useState("render");
	const [values, dispatchValues] = useCronReducer(defaultValue);
	const [error, onError] = useState<CronError>();
	const [clearButtonAction, setClearButtonAction] = useState<ClearButtonAction>("fill-with-every");
	const defaultAllowedDropdowns: CronType[] = ["period", "months", "month-days", "week-days", "hours", "minutes"];
	const [allowedDropdowns, setAllowedDropdowns] = useState<CronType[]>(defaultAllowedDropdowns);
	const defaultAllowedPeriods: PeriodType[] = ["year", "month", "week", "day", "hour", "minute", "reboot"];
	const [allowedPeriods, setAllowedPeriods] = useState<PeriodType[]>(defaultAllowedPeriods);
	...
	return (
		<div>
			<Form layout="inline" className="demo-dynamic-settings">
				...
				<Form.Item label="Allowed dropdowns">
					...
					>
						{defaultAllowedDropdowns.map((allowedDropdown) => {
							return <Select.Option key={allowedDropdown}>{allowedDropdown}</Select.Option>;
						})}
					</Select>
				</Form.Item>
				<Form.Item label="Allowed periods">
					...
					>
						{defaultAllowedPeriods.map((allowedPeriod) => {
							return <Select.Option key={allowedPeriod}>{allowedPeriod}</Select.Option>;
						})}
					</Select>
				</Form.Item>
				...
				<Form.Item label="Empty value management *">
					<Radio.Group value={allowEmpty} onChange={(e) => setAllowEmpty(e.target.value)}>
						<Radio.Button value="for-default-value">For default value</Radio.Button>
						<Radio.Button value="always">Always</Radio.Button>
						<Radio.Button value="never">Never</Radio.Button>
					</Radio.Group>
				</Form.Item>
				<Form.Item label="Default value *">
					<AntdInput value={defaultValue} onChange={(e) => setDefaultValue(e.target.value)} />
				</Form.Item>
				...
				<p>(**) Need to reset the component and to have an empty default value to see the changes</p>
			</Form>
			...
			{displayErrorText && <p style={{ marginTop: 20 }}>Error: {error ? error.description : "undefined"}</p>}
		</div>
	);
}
	...
	return (
		<div>
			...
			<div style={{ marginTop: 10 }}>
				<InfoCircleOutlined style={{ marginRight: 5 }} />
				<span style={{ fontSize: 12 }}>
					The &quot;onBlur&quot; event must be used instead of &quot;onChange&quot; to prevent a value change
					from the cron component
				</span>
			</div>
...
export function InputWithOnEnter() {
	...
	return (
		<div>
			...
			<div style={{ marginTop: 10 }}>
				<InfoCircleOutlined style={{ marginRight: 5 }} />
				<span style={{ fontSize: 12 }}>You can also add &quot;onEnter&quot; support to set the value</span>
			</div>
			<Divider>OR</Divider>
			<Cron
				value={values.cronValue}
...
export function DefaultValue() {
	...
	return (
		<div>
			...
			<div>
				<InfoCircleOutlined style={{ marginRight: 5 }} />
				<span style={{ fontSize: 12 }}>The first value will always be used as default value</span>
			</div>
		</div>
	);
}
...
export function DefaultPeriod() {
	...
	return (
		<div>
			...
			<div>
				<InfoCircleOutlined style={{ marginRight: 5 }} />
				<span style={{ fontSize: 12 }}>If not set, the prop &quot;defaultPeriod&quot; is &quot;day&quot;</span>
			</div>
		</div>
	);
}
...
export function Disabled() {
	...
	return (
		<div>
			...
			<div>
				<InfoCircleOutlined style={{ marginRight: 5 }} />
				<span style={{ fontSize: 12 }}>If not set, the prop &quot;disabled&quot; is false</span>
			</div>
		</div>
	);
}
...
export function ReadOnly() {
	...
	return (
		<div>
			...
			<div>
				<InfoCircleOutlined style={{ marginRight: 5 }} />
				<span style={{ fontSize: 12 }}>If not set, the prop &quot;readOnly&quot; is false</span>
			</div>
		</div>
	);
}
...
export function HumanizeLabels() {
	...
	return (
		<div>
			...
			<div>
				<InfoCircleOutlined style={{ marginRight: 5 }} />
				<span style={{ fontSize: 12 }}>If not set, the prop &quot;humanizeLabels&quot; is true</span>
			</div>
			<div>
				<InfoCircleOutlined style={{ marginRight: 5 }} />
				<span style={{ fontSize: 12 }}>Humanizes the labels in the cron component</span>
			</div>
			<div>
				<InfoCircleOutlined style={{ marginRight: 5 }} />
				<span style={{ fontSize: 12 }}>Works only for week days and months</span>
			</div>
			<div>
				<InfoCircleOutlined style={{ marginRight: 5 }} />
				<span style={{ fontSize: 12 }}>
					Can be used with &quot;locale&quot; prop and &quot;altMonths&quot; / &quot;altWeekDays&quot;
					properties in order to display translated labels
				</span>
			</div>
		</div>
	);
...
export function HumanizeValue() {
	...
	return (
		<div>
			...
			<div>
				<InfoCircleOutlined style={{ marginRight: 5 }} />
				<span style={{ fontSize: 12 }}>If not set, the prop &quot;humanizeValue&quot; is false</span>
			</div>
			<div>
				<InfoCircleOutlined style={{ marginRight: 5 }} />
				<span style={{ fontSize: 12 }}>
					The prop &quot;humanizeValue&quot; cannot be used to prohibit used of valid string value like
					&quot;MON,WED&quot;
				</span>
			</div>
			<div>
				<InfoCircleOutlined style={{ marginRight: 5 }} />
				<span style={{ fontSize: 12 }}>
					If the prop &quot;humanizeValue&quot; is true, the component will automatically convert a valid
			...
			<div>
				<InfoCircleOutlined style={{ marginRight: 5 }} />
				<span style={{ fontSize: 12 }}>
					If the prop &quot;humanizeValue&quot; is false, the component will automatically convert a valid
					string value to number
				</span>
			</div>
			<div>
				<InfoCircleOutlined style={{ marginRight: 5 }} />
				<span style={{ fontSize: 12 }}>A valid string value can be in lowercase or uppercase</span>
			</div>
			<div>
				<InfoCircleOutlined style={{ marginRight: 5 }} />
				<span style={{ fontSize: 12 }}>Works only for week days and months</span>
...
export function HumanizeLabelsAndValue() {
	...
	return (
		<div>
			...
			<div>
				<InfoCircleOutlined style={{ marginRight: 5 }} />
				<span style={{ fontSize: 12 }}>Humanizes in the cron component both the labels and the value</span>
			</div>
			<div>
				<InfoCircleOutlined style={{ marginRight: 5 }} />
				<span style={{ fontSize: 12 }}>Works only for week days and months</span>
	...
	return (
		<div>
			...
			<div>
				<InfoCircleOutlined style={{ marginRight: 5 }} />
				<span style={{ fontSize: 12 }}>
					You can set the prop to a boolean or an array [&quot;minutes&quot;, &quot;hours&quot;,
					&quot;month-days&quot;]
				</span>
			</div>
			<div>
				<InfoCircleOutlined style={{ marginRight: 5 }} />
				<span style={{ fontSize: 12 }}>If not set, the prop &quot;leadingZero&quot; is &quot;never&quot;</span>
			</div>
		</div>
	);
}
	...
	return (
		<div>
			...
			<div style={{ marginTop: 10 }}>
				<InfoCircleOutlined style={{ marginRight: 5 }} />
				<span style={{ fontSize: 12 }}>
					Write a bad cron expression to trigger an error after the &quot;onBlur&quot; event
				</span>
			</div>
...
export function ClearButtonEmptyValue() {
	...
	return (
		<div>
			...
			<Cron value={value} setValue={setValue} clearButtonAction={clearButtonAction} />

			<div>
				<InfoCircleOutlined style={{ marginRight: 5 }} />
				<span style={{ fontSize: 12 }}>
					The &quot;clearButtonAction&quot; prop allow you to empty the field or fill it with &quot;* * * *
					*&quot;
				</span>
			</div>
			<div>
	...
	return (
		<div>
			...
			<div>
				<InfoCircleOutlined style={{ marginRight: 5 }} />
				<span style={{ fontSize: 12 }}>
					If not set, the prop &quot;Shortcuts&quot; is [&quot;@yearly&quot;, &quot;@annually&quot;,
...
export function NoPeriodicityOnDoubleClick() {
	...
	return (
		<div>
			...
			<Cron value={value} setValue={setValue} periodicityOnDoubleClick={false} />

			<div>
				<InfoCircleOutlined style={{ marginRight: 5 }} />
				<span style={{ fontSize: 12 }}>
					You can disable the double click on a dropdown option that automatically select / unselect a
					periodicity
				</span>
			</div>
		</div>
	...
	return (
		<div>
			...
			<div>
				<InfoCircleOutlined style={{ marginRight: 5 }} />
				<span style={{ fontSize: 12 }}>
					If you want to disable the double click on a dropdown option that automatically select / unselect a
					periodicity, set &quot;periodicityOnDoubleClick&quot; prop at false
				</span>
			</div>
		</div>
	);
...
export function SingleSelectionModeAutoClose() {
	...
	return (
		<div>
			...
			<Cron value={value} setValue={setValue} mode="single" periodicityOnDoubleClick={false} />

			<div>
				<InfoCircleOutlined style={{ marginRight: 5 }} />
				<span style={{ fontSize: 12 }}>
					When single mode is active and &quot;periodicityOnDoubleClick&quot; is false, each dropdown will
					automatically close after selecting a value
				</span>
			</div>
			<div>
				<InfoCircleOutlined style={{ marginRight: 5 }} />
...
export function RestrictPeriodAndSelection() {
	...
	return (
		<div>
			...
			<p>allowedPeriods: [&quot;year&quot;, &quot;month&quot;, &quot;week&quot;, &quot;minute&quot;]</p>
			...
			<div>
				<InfoCircleOutlined style={{ marginRight: 5 }} />
				<span style={{ fontSize: 12 }}>
					E.g.: Hide hours and minutes by removing them from &quot;allowedDropdowns&quot; prop.
				</span>
			</div>
			<div>
				<InfoCircleOutlined style={{ marginRight: 5 }} />
				<span style={{ fontSize: 12 }}>
					E.g.: Also hide &quot;day&quot; and &quot;hour&quot; periods with &quot;allowedPeriods&quot; because
					hours and minutes cannot be changed.
				</span>
			</div>
		</div>
	);
	...
	return (
		<div>
			...
			<div>
				<InfoCircleOutlined style={{ marginRight: 5 }} />
				<span style={{ fontSize: 12 }}>
					The order of the &quot;locale&quot; properties &quot;weekDays&quot;, &quot;months&quot;,
					&quot;altMonths&quot; and &quot;altWeekDays&quot; is important! The index will be used as value
				</span>
			</div>
			<div>
				<InfoCircleOutlined style={{ marginRight: 5 }} />
				<span style={{ fontSize: 12 }}>
					Sunday must always be the first value of &quot;weekDays&quot; and &quot;altWeekDays&quot; property
					because it&apos;s &quot;0&quot;
				</span>
			</div>
		</div>
...
export function NoPrefixAndSuffix() {
	...
	return (
		<div>
			...
			<Cron locale={NO_PREFIX_SUFFIX_LOCALE} value={value} setValue={setValue} />
...
export function DropdownsConfig() {
	...
	return (
		<div>
			...
			<div>
				<InfoCircleOutlined style={{ marginRight: 5 }} />
				<span style={{ fontSize: 12 }}>You can configure each dropdown individually</span>
			</div>
		</div>
	);
}
...
export function CustomStyle() {
	...
	return (
		<div>
			...
			<div>
				<InfoCircleOutlined style={{ marginRight: 5 }} />
				<span style={{ fontSize: 12 }}>Custom CSS example (See file &quot;styles.stories.css&quot;):</span>
				<ul>
			...
			<div>
				<InfoCircleOutlined style={{ marginRight: 5 }} />
				<span style={{ fontSize: 12 }}>Available classes when using the prop &quot;className&quot;:</span>
				<ul>
			...
			<div>
				<InfoCircleOutlined style={{ marginRight: 5 }} />
				<span style={{ fontSize: 12 }}>Don&apos;t forget to import antd default style</span>
			</div>
		</div>
	);
}
