...
import MinuteSteps from "./fields/MinuteSteps";
...
export default function Cron(props: CronProps) {
	const {
		...
		mode = "multiple",
		allowedDropdowns = ["period", "months", "month-days", "week-days", "hours", "minutes", "minutes-steps"],
		allowedPeriods = ["year", "month", "week", "day", "hour", "minute", "minutes-steps", "reboot"],
		allowClear,
		dropdownsConfig
	} = props;
	...
	const temp = useState<PeriodType | undefined>();

	const period = temp[0];
	const setPeriod: any = temp[1];
	...
	const [minutesSteps, setMinutesSteps] = useState<number[] | undefined>();
	...
	useEffect(
		() => {
			...
			if (
				(period || minutes || months || monthDays || weekDays || hours || minutesSteps) &&
				!valueCleared &&
			...
			{periodForRender === "reboot" ? (
				...
				<>
					{periodForRender === "year" && allowedDropdowns.includes("months") && (
						<Months
							...
							periodicityOnDoubleClick={
								dropdownsConfig?.months?.periodicityOnDoubleClick ?? periodicityOnDoubleClick
							}
					...
					{(periodForRender === "year" || periodForRender === "month" || periodForRender === "week") &&
						allowedDropdowns.includes("week-days") && (
							<WeekDays
								...
								period={periodForRender}
								periodicityOnDoubleClick={
									dropdownsConfig?.["week-days"]?.periodicityOnDoubleClick ?? periodicityOnDoubleClick
								}
					...
					<div>
						{periodForRender !== "minute" &&
							...
							allowedDropdowns.includes("hours") && (
								<Hours
									...
									period={periodForRender}
									periodicityOnDoubleClick={
										dropdownsConfig?.hours?.periodicityOnDoubleClick ?? periodicityOnDoubleClick
									}
						...
						{periodForRender === "minutes-steps" && allowedDropdowns.includes("minutes-steps") && (
							<MinuteSteps
								value={minutesSteps}
								setValue={setMinutesSteps}
								locale={locale}
								className={className}
								disabled={dropdownsConfig?.["minutes-steps"]?.disabled ?? disabled}
								readOnly={dropdownsConfig?.["minutes-steps"]?.readOnly ?? readOnly}
								leadingZero={dropdownsConfig?.["minutes-steps"]?.leadingZero ?? leadingZero}
								clockFormat={clockFormat}
								period={periodForRender}
								periodicityOnDoubleClick={
									dropdownsConfig?.["minutes-steps"]?.periodicityOnDoubleClick ??
									periodicityOnDoubleClick
								}
								mode={dropdownsConfig?.["minutes-steps"]?.mode ?? mode}
								allowClear={dropdownsConfig?.["minutes-steps"]?.allowClear ?? allowClear}
								filterOption={dropdownsConfig?.["minutes-steps"]?.filterOption}
							/>
						)}
						...
						{periodForRender !== "minute" &&
							...
							allowedDropdowns.includes("minutes") && (
								<Minutes
									...
									clockFormat={clockFormat}
									periodicityOnDoubleClick={
										dropdownsConfig?.minutes?.periodicityOnDoubleClick ?? periodicityOnDoubleClick
									}
