...
export function setValuesFromCronString(
	...
	locale: Locale,
	shortcuts: Shortcuts,
	setMinutesSteps: SetValueNumbersOrUndefined,
	...
	if (!error) {
		...
		try {
			...
			cronString.includes("/") ? setMinutesSteps(cronParts[0]) : setMinutes(cronParts[0]);
...
export function getCronStringFromValues(
	...
	if (period === "minutes-steps" && minuteSteps) {
		return parseCronMinutesSteps(minuteSteps);
	}
	...
	const newHours = period !== "minute" && period !== "hour" && period !== "minutes-steps" && hours ? hours : [];
	const newMinutes = period !== "minute" && period !== "minutes-steps" && minutes ? minutes : [];
 ...
 * Parses a 2-dimensional array of integers as a cron schedule
 */
function parseCronMinutesSteps(minuteSteps: number[]) {
	const start = minuteSteps.length ? "*/" : "*";

	const endString = " * * * *";

	return start + minuteSteps.join(",") + endString;
}
